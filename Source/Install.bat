...
rem IF EXIST "%RIMWORLD_DIR_STANDALONE%" (
rem 	ECHO "Copying to %TARGET_DEBUG_DIR%"
rem 	IF NOT EXIST "%TARGET_DEBUG_DIR%" MKDIR "%TARGET_DEBUG_DIR%" 1>NUL
rem 	FOR %%D IN (%~4) DO (
rem 		XCOPY /I /Y /E "%SOLUTION_DIR%%%D" "%TARGET_DEBUG_DIR%\%%D" 1>NUL
rem 	)
rem 	FOR %%D IN (%~5) DO (
rem 		XCOPY /Y "%SOLUTION_DIR%%%D" "%TARGET_DEBUG_DIR%\*" 1>NUL
rem 	)
rem )

        ...
        public static IEnumerable<ThingDef> ImpliedBloodDefs()
        {
            foreach (ThingDef sourceDef in DefDatabase<ThingDef>.AllDefs.ToList())
            {
                ...
                if (sourceDef.race.Humanlike)
                {
                    ...
                    bloodDef.comps.Add(new CompProperties_UsableWithSkill
                    {
                    ...
                    bloodDef.comps.Add(new CompProperties_UsableByRace { allowedRaces = new List<ThingDef>{ sourceDef } });
                    bloodDef.comps.Add(new CompProperties_UseEffect { compClass = typeof(CompUseEffect_GiveBlood) });
                ...
                {
                    bloodDef.label = "InsectBloodLabel".Translate();
                }
                else
                {
                    ...
                    if (!meatLabel.NullOrEmpty() && meatLabel.Contains(' '))
                    {
                        ...
                        bloodDef.label = "BloodLabel".Translate(meatLabel.Substring(0, meatLabel.IndexOf(' ')));
                    }
                    else
                    {
                        //no meat label, or meat label is for common meat name. just use the source def label
                        bloodDef.label = "BloodLabel".Translate(sourceDef.label);
                    }
                }
